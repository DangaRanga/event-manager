<template>
  <div class="grid grid-cols-2 my-5" >
    <article v-if="isdata">
      <img :src="event_data.photo" id="event-image" alt="event-image" />
    </article>
    <section id="event-content">
      <h1 class="text-4xl font-bold my-3"><a :href="event_data.website_url">{{event_data.title}}</a></h1>
      <p class="my-4 w-4/5">
      ...
      <h5 class="text-gray-600 my-2" v-if="isdata">
        <div>
          <svg class="h-8 w-8 text-red-500"  width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">  
            ...
            <line x1="4" y1="11" x2="20" y2="11" />  <rect x="8" y="15" width="2" height="2" />
          </svg>
          <span>Date: <span>{{event_data.start_date}}</span> -<span>{{event_data.end_date}}</span></span>
        </div>
      </h5>
      ...
      <h5 class="text-gray-600 my-2" v-if="isdata">
        <div>
          ...
          <span>Time: <span>{{event_data.start_time}}</span> - <span>{{event_data.end_time}}</span></span>
        </div>
      </h5>

      <button v-if="isdata"
        class=" w-1/4 px-7 py-3 bg-primary text-white rounded text-sm font-medium hover:bg-primaryHover transition ease-in-out delay-150"
      >
        Register
      </button>
    </section>
